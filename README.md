...
- `xorg-server-xvfb` — invisible Netflix login (no HDMI). Without it, login uses hidden gamescope.
...
Harvest **and** phone login share one invisible nest: the same Chrome profile, **no gamescope** (a second DRM nest would steal HDMI), **no `-f`**, `--start-minimized` + off-screen window, then hide and raise the guide. Play still wraps gamescope fullscreen. See [`docs/companion.md`](docs/companion.md).
...
This is Zappe HID mapping, not a Samsung TV API. Whisper: `ZAPPE_WHISPER_BIN` (`-l pt`). `ZAPPE_VOICE_FAKE=abrir netflix` for tests. Phone companion + device codes are the login path — never on-TV typing.
