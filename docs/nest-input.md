...
| Back / Home | — | **Return to guide** (sibling PR). Not injected as Netflix Back | Stop mpv |

Back / Home are not handled here. Shared helpers: `NestManager::send_key`, `nest_window_pids` (inner gamescope only), `nest_input::inject_key`.
...
5. Back / Home still return to the guide (other PR). Do not expect Netflix in-app Back from those keys.
