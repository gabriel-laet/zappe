#!/usr/bin/env python3
"""zappe-atongx-capture — dump ATONGX / XING WEI EV_KEY + MSC_SCAN lines.

60-second couch path on zappe-tv:

  ./packaging/appliance/zappe-atongx-capture --mic-pointer

Press Mic (red), then Pointer. Paste the two lines back into
src/lib/atongx-map.json if they differ from the locked KEY_SEARCH /
KEY_VOICECOMMAND / KEY_F2 / KEY_TOUCHPAD_TOGGLE contract.
...
def action_for_code(spec: dict, code: int) -> tuple[str | None, str | None, str | None]:
    for binding in spec.get("bindings", []):
        for key in binding.get("keys", []):
            if int(key.get("code", -1)) == code:
                return binding.get("action"), key.get("name"), key.get("confidence")
    return None, None, None


def key_name(code: int, spec: dict) -> str:
    _, mapped, _ = action_for_code(spec, code)
    if mapped:
        return mapped
...
def listen(blocks: list[dict], spec: dict, remaining: list[str], include_mouse: bool) -> int:
    targets = [
        b
        for b in blocks
        if b.get("path") and (include_mouse or iface_of(b) != "mouse")
    ]
    fds: dict[int, dict] = {}
    ...
    if not fds:
        ...
        print("# evtest fallback:", file=sys.stderr)
        print("#   cat /proc/bus/input/devices | less", file=sys.stderr)
        print("#   sudo evtest /dev/input/event5   # consumer — Mic", file=sys.stderr)
        print("#   sudo evtest /dev/input/event3   # keyboard — Pointer", file=sys.stderr)
        return 2

    if remaining:
        print("# Walk the prompted buttons. Ctrl-C ends.")
        print(f"# next → {remaining[0]}")
        print("# If Pointer prints no EV_KEY, wave the remote — REL_* means firmware-local gyro.")
    else:
            ...
            for fd in ready:
                ...
                if etype == EV_REL:
                    if not saw_rel:
                        ...
                        print(
                            f"[{stamp}] {meta['block'].get('event')} mouse     "
                            f"REL code={code} value={value}  → pointer-motion "
                            "(no EV_KEY — gyro may be firmware-local)",
                            flush=True,
                        )
                    continue
                if etype != EV_KEY:
                ...
                action, _, confidence = action_for_code(spec, code)
                ...
                if action:
                    ...
                    extra = "  → (unmapped)  << paste this line back"
...
def main() -> int:
    ...
    parser.add_argument(
        "--mic-pointer",
        action="store_true",
        help="60s path: prompt only Mic (red) then Pointer",
    )
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    remaining: list[str] = []
    if args.mic_pointer:
        remaining = ["Mic (red)", "Air-mouse cursor toggle"]
    elif args.prompt:
        remaining = [b.get("button") for b in (spec.get("bindings") or [])]
    return listen(
        inventory or chosen,
        spec,
        remaining=remaining,
        include_mouse=args.mic_pointer,
    )
