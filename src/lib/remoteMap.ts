 ...
 * Consumer Control keys (Back / Home / Play-Pause / Menu / Mic) do not
 * register as Tauri global shortcuts (`Unknown scancode`); Rust reads those
 * from evdev instead (`hid.rs`).
 *
 * Volume and mute go to the Samsung TV (Device Connect), then `pactl` if the
 * TV is unreachable. Power returns to the guide and never shuts the box down.
 * Pointer toggles the CSS cursor on the guide and a real nest cursor while
 * Chrome is playing (see nest-input.md).
 */
...
export type AtongxConfidence = "hid" | "alias" | "locked";
